...
describe( 'HKDF', function() {
    ...
    it( 'test vector 1', function( done ) {
        as.add(
            ( as ) => {
                const hkdf = VaultPlugin.getPlugin( 'HKDF' );
                hkdf.derive(
                    as,
    ...
    it( 'test vector 2', function( done ) {
        as.add(
            ( as ) => {
                const hkdf = VaultPlugin.getPlugin( 'HKDF' );
                hkdf.derive(
                    as,
                    Buffer.from( '000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f404142434445464748494a4b4c4d4e4f', 'hex' ),
                    82 * 8,
                    'SHA-256',
                    {
    ...
    it( 'test vector 3', function( done ) {
        as.add(
            ( as ) => {
                const hkdf = VaultPlugin.getPlugin( 'HKDF' );
                hkdf.derive(
    ...
    it( 'test vector 4', function( done ) {
        as.add(
            ( as ) => {
                const hkdf = VaultPlugin.getPlugin( 'HKDF' );
                hkdf.derive(
                    as,
                    Buffer.from( '0b0b0b0b0b0b0b0b0b0b0b', 'hex' ),
                    42 * 8,
                    'SHA-1',
                    {
                        salt: Buffer.from( '000102030405060708090a0b0c', 'hex' ),
    ...
    it( 'test vector 6', function( done ) {
        as.add(
            ( as ) => {
                const hkdf = VaultPlugin.getPlugin( 'HKDF' );
                hkdf.derive(
    ...
    it( 'test vector 7', function( done ) {
        as.add(
            ( as ) => {
                const hkdf = VaultPlugin.getPlugin( 'HKDF' );
                hkdf.derive(
                    as,
                    Buffer.from( '0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c', 'hex' ),
                    42 * 8,
                    'SHA-1',
                    {
                        salt: '',
