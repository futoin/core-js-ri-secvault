...
class EncryptedStorage extends Storage {
    ...
    _encrypt( as, uuidb64, key_info ) {
        ...
        const opts = Object.assign(
            ...
            {
                // natural unique IV
                iv: Buffer.from( uuidb64, 'base64' ),
                iv_length: FORCED_IV_LENGTH,
            }
        );

        this._enc_plugin.encrypt( as, kek, key_info.raw, opts );
        as.add( ( as, edata ) => key_info.data = edata.toString( 'base64' ) );
    }

    _decrypt( as, _uuidb64, key_info ) {
        const kek = g_keks.get( this );
        const opts = Object.assign(
            {},
            this._cipher_opts,
