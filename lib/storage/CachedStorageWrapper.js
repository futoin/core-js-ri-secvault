...
const LiveReceiver = require( 'futoin-eventstream/LiveReceiver' );
...
class CachedStorageWrapper extends Storage {
    ...
    loadExt( as, ext_id, decrypt=true ) {
        ...
        // Ensure sync & touch
        if ( ki !== undefined && this[KEY_CACHE].get( ki.uuidb64 ) ) {
            as.successStep( ki );
        } else {
    ...
    remove( as, uuidb64 ) {
        ...
        as.add( ( as ) => {
            ...
            const ki = key_cache.get( uuidb64 );

            if ( ki !== undefined ) {
                key_cache.delete( uuidb64 );
                key_cache.dispose( uuidb64, ki );
            }
        } );
    }
    ...
    _startCache() {
        ...
        const receiver = new class extends LiveReceiver {
            constructor() {
                super( ccm );
            }
