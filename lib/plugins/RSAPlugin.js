...
class RSAPlugin extends VaultPlugin {
    ...
    encrypt( as, key, data ) {
        const res = crypto.publicEncrypt( key.toString(), data );
        as.add( ( as ) => as.success( res ) );
    }

    decrypt( as, key, data ) {
        const res = crypto.privateDecrypt( key.toString(), data );
        as.add( ( as ) => as.success( res ) );
    }
    ...
    sign( as, key, data, options = {} ) {
        ...
        sign.update( data );
        const res = sign.sign( key.toString() );
        as.add( ( as ) => as.success( res ) );
    }
    ...
    verify( as, key, data, sig, options = {} ) {
        ...
        verify.update( data );
        const res = verify.verify( key.toString(), sig );
